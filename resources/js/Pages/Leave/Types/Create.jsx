...
export default function Create() {
    const { data, setData, post, processing, errors } = useForm({
        name: '', days_per_year: 0, is_paid: true, requires_approval: true, is_active: true,
    });

    return (
        <AuthenticatedLayout header={<h2 className="text-xl font-semibold text-gray-800">New Leave Type</h2>}>
            <Head title="New Leave Type" />
            <div className="py-8 max-w-xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="bg-white shadow rounded-xl p-6">
                    <form onSubmit={e => { e.preventDefault(); post(route('leave-types.store')); }} className="space-y-5">
                        <div>
                        ...
                        <div className="flex justify-end gap-3 pt-2">
                            ...
                            <button type="submit" disabled={processing} className="px-5 py-2 bg-indigo-600 text-white text-sm font-medium rounded-lg hover:bg-indigo-700 disabled:opacity-50 transition">Create</button>
                        </div>
                    </form>
                </div>
            </div>
