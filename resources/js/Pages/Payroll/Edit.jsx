...
export default function Edit({ payroll, employees }) {
    const { data, setData, patch, processing, errors } = useForm({
        ...
        period_start: (payroll.period_start || '').split('T')[0],
        period_end: (payroll.period_end || '').split('T')[0],
    ...
    const submit = (e) => { e.preventDefault(); patch(route('payroll.update', payroll.id)); };

    return (
        <AuthenticatedLayout header={<h2 className="text-xl font-semibold text-gray-800">Edit Payroll Entry</h2>}>
            <Head title="Edit Payroll Entry" />
